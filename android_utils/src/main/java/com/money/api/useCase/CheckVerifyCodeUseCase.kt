...
class CheckVerifyCodeUseCase(
    ...
    suspend operator fun invoke(agent: String, verifyCode: String): CheckVerifyCodeState {
        try {
            ...
            return when (val code = jsonObject.get("statusCode").asInt) {
                ...
                405 -> {
                    ...
                    CheckVerifyCodeState.OnError(IllegalStateException("$tag -> 未知狀態碼: $code"))
                }
            }
        } catch (exception: Exception) {
            return CheckVerifyCodeState.OnError(IllegalStateException("$tag -> occur exception: $exception"))
        }
    }
}